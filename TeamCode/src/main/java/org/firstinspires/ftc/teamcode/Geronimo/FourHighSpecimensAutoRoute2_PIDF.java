...
@Autonomous(name="Four High Specimens Auto Route 2.0 PIDF", preselectTeleOp =
        "Geronimo 1 Manual Control")
@Disabled
public class FourHighSpecimensAutoRoute2_PIDF extends LinearOpMode {
...
*/
        ...
        // ***************************************************
        Actions.runBlocking(
                // Run a parallel action the whole time that is setting the Rotator Arm Control using PIDF values
                new ParallelAction(managePidfArmControl(),
...
*/
    ...
    public Action setReadyToEnd() { return new SetReadyToEnd(); }
    public class SetReadyToEnd implements Action {
        @Override
        public boolean run(@NonNull TelemetryPacket packet) {
            readyToEnd = true;
            return false;  // returning true means not done, and will be called again.  False means action is completely done
    ...
    public Action managePidfArmControl () { return new ManagePIDF_ArmControl(); }
